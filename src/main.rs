...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token_rotator = core::TokenRotator::new(config.github_tokens.clone());
...
fn print_config_summary(config: &config::Config) {
    ...
    info!("  ├─ GitHub Tokens: {}", 
        if !config.github_tokens.is_empty() { 
            format!("{} tokens", config.github_tokens.len()) 
        } else { 
            "❌ Not set".to_string() 
        });
    info!("  ├─ Telegram: {}", 
    ...
    info!("  ├─ Poll Interval: {}s (fallback)", config.poll_interval_secs);
